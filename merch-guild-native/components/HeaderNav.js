  ...
  return (
    <>
      <View style={styles.headerButtonsView}>
        ...
        {Auth.loggedIn() ? (
          <>
            {others.filter((i) => i != 'Login').map((k, l) => {
              return (
                <Pressable
            ...
            {others.filter((i) => i === 'Search').length ? <>
              <Pressable style={styles.headerButtons} onPress={() => navRef.navigate('Search')}><Text style={[styles.text, { fontSize: 12 }]}>Search</Text></Pressable>
            </> : <>
              <Pressable style={styles.headerButtons} onPress={() => navRef.navigate('Home')}><Text style={[styles.text, { fontSize: 12 }]}>About</Text></Pressable>
            </>}
